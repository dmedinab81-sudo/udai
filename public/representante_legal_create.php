...
if ($_SERVER['REQUEST_METHOD']==='POST') {
	...
	if (empty($errors)) {
		$pdo=getPDO();
		try {
			$stmt=$pdo->prepare("INSERT INTO representante_legal (cedula,nombres,telefono,correo) VALUES (?,?,?,?)");
			$stmt->execute([$cedula,$nombres,$telefono,$correo]);
			set_flash('Representante creado.');
...
<form method="post">
  ...
  <div class="mb-3">
    ...
    <input class="form-control" name="nombres" id="nombres" value="<?= htmlspecialchars($_POST['nombres'] ?? '') ?>">
  </div>
  ...
  <div class="mb-3"><label class="form-label">Correo</label>
    <input class="form-control" name="correo" value="<?= htmlspecialchars($_POST['correo'] ?? '') ?>">
  </div>
