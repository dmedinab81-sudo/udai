...
$pdo = getPDO();
$id = intval($_GET['id'] ?? 0);
$stmt = $pdo->prepare("SELECT * FROM docente_tutor WHERE id = ? LIMIT 1");
$stmt->execute([$id]);
$row = $stmt->fetch();
if (!$row) { header('Location: docente_tutor.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($token)) {
        ...
        header('Location: docente_tutor.php'); exit;
    }
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare(
            "UPDATE docente_tutor SET cedula=?, nombres=?, telefono=?, correo=? WHERE id=?"
        );
        $stmt->execute([$cedula,$nombres,$telefono,$correo,$id]);
        set_flash('Docente tutor actualizado.');
        header('Location: docente_tutor.php'); exit;
    }
}
  ...
  <div class="col-md-6 mx-auto">
    ...
    <form method="post">
      ...
      <div class="mb-3">
       ...
       required
       oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,10)"
       value="<?= htmlspecialchars((isset($_POST['telefono']) && $_POST['telefono'] !== '') ? $_POST['telefono'] : $row['telefono']) ?>">
      </div>
      ...
      <a class="btn btn-secondary" href="docente_tutor.php">Cancelar</a>
    </form>
  </div>
</div>
...
document.getElementById('btn_buscar_rc').addEventListener('click', function () {
  ...
  buscarRC(cedula, function (err, data) {
    if (err || !data || !data.success) {
      setEstado('No se pudo consultar.');
      return;
    }
