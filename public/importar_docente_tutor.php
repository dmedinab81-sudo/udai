<?php
/**
 * importar_docente_tutor.php
 * Importa docentes tutor desde un CSV delimitado por ; a la tabla docente_tutor
...
// ===== CONFIG =====
$csvPath   = __DIR__ . '/uploads/docente_tutor.csv'; // cambia el nombre si tu CSV se llama distinto
$delimiter = ';';
...
function limpiarCedula(?string $c): ?string {
    ...
    // solo dígitos
    $c = preg_replace('/\D+/', '', $c);
    // acepta 10 dígitos (cédula) - si quieres aceptar otros largos, quita esta línea
...
function limpiarCorreo(?string $mail): ?string {
    ...
    // quitar espacios y normalizar a minúsculas
    ...
    if (!preg_match('/^[A-Za-z0-9._%+\-]+@docentes\.educacion\.edu\.ec$/', $mail)) {
        // si prefieres permitir cualquier correo, comenta este return null
        return null;
    }
    return $mail;
}
...
$sql = "
INSERT INTO docente_tutor (cedula, nombres, telefono, correo)
...
try {
    while (($row = fgetcsv($handle, 0, $delimiter)) !== false) {
        ...
        // CSV trae: CEDULA;NOMBRES;TELEFONO;CORREO
        ...
        // cedula obligatoria para deduplicar
        if ($cedula === null) {
            $omitidos++;
            continue;
        }

        // si quieres obligar correo institucional válido, descomenta:
        // if ($correo === null) { $omitidos_por_correo++; continue; }
        ...
        else $actualizados++; // tratado como procesado
    }
    ...
    // echo "Omitidos (correo no institucional): {$omitidos_por_correo}\n";
